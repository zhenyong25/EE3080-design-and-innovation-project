...
The Espdrone client requires [edlib](https://github.com/bitcraze/espdrone-lib-python).
...
Pycharm is an IDE for python. Any Python IDE or development environment will work for the Espdrone client. To work on the Espdrone firmware with Pycharm, install pycharm community edition and open the Espdrone client folder in it. Pycharm will automatically detect the python installation.

To run the client, open and run the file ```bin/edclient```.
...
Now you can run the client with ```build\exe.win32-3.6\edclient.exe```.
...
It should be possible to package the client as a mac app and [help is wanted](https://github.com/bitcraze/espdrone-clients-python/issues/231).
...
If you are using Anaconda/Conda, the procedure should be very similar but you can skip the python installation.
 ...
 3) Open a console and install the client with ```python3 -m pip install edclient[qt5] pysdl2```. This will install the latest release.

You can now launch the client with ```python3 -m edclient.gui```

If you want to develop and modify the client, you can clone this repos, uninstall edclient with ```python3 -m pip uninstall edclient``` and install it in development mode by navigating into the repos root folder and installing the client in edit mode: ```python3 -m pip install -e .```.
...
1. Install the client
    ...
    * If you only want to use the client to fly the Espdrone and don't care about coding
    ```
    pip3 install edclient
    ```
    ...
    * If you want to develop the client and play with the source code. From the source folder run
    ...
    pip3 install -e .
    ```
    If you want to develop on edlib as well, install edlib from <https://github.com/bitcraze/espdrone-lib-python>
...
To launch the GUI after a systemwide installation, execute ```edclient```.
...
Using Crazyradio on Linux requires that you set udev permissions. See the edlib
[readme](https://github.com/bitcraze/espdrone-lib-python#setting-udev-permissions)
