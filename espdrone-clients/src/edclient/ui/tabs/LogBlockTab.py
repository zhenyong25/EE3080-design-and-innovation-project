...
from edclient.utils.logdatawriter import LogWriter

__author__ = 'Bitcraze AB'
__all__ = ['LogBlockTab']

logblock_tab_class = uic.loadUiType(
    edclient.module_path + "/ui/tabs/logBlockTab.ui")[0]
...
class LogBlockItem(object):
    ...
    def _log_error(self, logconfig, msg):
        """
        Callback when there's an error starting the block in the Espdrone
        """
    ...
    def _set_started(self, conf, started):
        """Callback when a block has been started in the Espdrone"""
    ...
    def _set_added(self, conf, started):
        """Callback when a block has been added to the Espdrone"""
