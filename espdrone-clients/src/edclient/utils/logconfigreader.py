...
points to the Espdrone. It will also accept settings from the UI.
...
import edclient
from edlib.espdrone.log import LogVariable, LogConfig
...
class LogConfigReader():
    ...
    def __init__(self, espdrone):
        ...
        if (not os.path.exists(edclient.config_path + "/log")):
            logger.info("No user config found, copying dist files")
            os.makedirs(edclient.config_path + "/log")
            for f in glob.glob(
                    edclient.module_path + "/configs/log/[A-Za-z]*.json"):
                shutil.copy2(f, edclient.config_path + "/log")
        self._cf = espdrone
    ...
    def _read_config_files(self):
        ...
        configsfound = [os.path.basename(f) for f in
                        glob.glob(edclient.config_path +
                                  "/log/[A-Za-z_-]*.json")]
        new_dsList = []
    ...
    def _connected(self, link_uri):
        """Callback that is called once Espdrone is connected"""
    ...
    def saveLogConfigFile(self, logconfig):
        ...
        filename = edclient.config_path + "/log/" + logconfig.name + ".json"
