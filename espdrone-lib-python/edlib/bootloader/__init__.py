...
#  MA  02110-1301, USA.
"""
Bootloading utilities for the Espdrone.
"""
import json
import logging
import sys
...
class Bootloader:
    """Bootloader utility for the Espdrone"""
    ...
    def start_bootloader(self, warm_boot=False):
        ...
        if started:
            ...
            if (self.protocol_version == BootVersion.ed1_PROTO_VER_0 or
                    self.protocol_version == BootVersion.ed1_PROTO_VER_1):
                # Nothing more to do
                pass
            elif self.protocol_version == BootVersion.ed2_PROTO_VER:
    ...
    def write_ed1_config(self, data):
        target = self._cload.targets[0xFF]
        config_page = target.flash_pages - 1

        to_flash = {'target': target, 'data': data, 'type': 'ed1 config',
    ...
    def reset_to_firmware(self):
        if self._cload.protocol_version == BootVersion.ed2_PROTO_VER:
    ...
    def _get_platform_id(self):
        ...
        identifier = 'ed1'
        if (BootVersion.is_ed2(self.protocol_version)):
            identifier = 'ed2'
