...
#  MA  02110-1301, USA.
"""
Enables logging of variables from the Espdrone.

When a Espdrone is connected it's possible to download a TableOfContent of all
...
States of a configuration:
  ...
  Created on ed   - When the configuration is deemed valid it is added to the
                    Espdrone. At this time the memory constraint is checked
                    and the status returned.
  Started on ed   - Any added block that is not started can be started. Once
                    started the Espdrone will send back logdata periodically
                    according to the specified period when it's created.
  Stopped on ed   - Any started configuration can be stopped. The memory taken
                    by the configuration on the Espdrone is NOT freed, the
                    only effect is that the Espdrone will stop sending
                    logdata back to the host.
  Deleted on ed   - Any block that is added can be deleted. When this is done
                    the memory taken by the configuration is freed on the
                    Espdrone. The configuration will have to be re-added to
                    be used again.
"""
import errno
import logging
        ...
        for var in self.variables:
            if (var.is_toc_variable() is False):  # Memory location
                ...
                logger.debug('Adding %s with id=%d and type=0x%02X',
                             var.name,
                             self.ed.log.toc.get_element_id(
