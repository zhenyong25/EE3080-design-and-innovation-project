...
#  MA  02110-1301, USA.
"""
Enables flash access to the Espdrone.
...
class LEDDriverMemory(MemoryElement):
    ...
    def write_data(self, write_finished_cb):
        """Write the saved LED-ring data to the Espdrone"""
...
class LighthouseMemory(MemoryElement):
    ...
    def write_data(self, write_finished_cb):
        """Write geometry data to the Espdrone"""
        self._write_finished_cb = write_finished_cb
        data = bytearray()
...
class MemoryTester(MemoryElement):
    ...
    2. To verify writing:
      * Set the parameter 'memTst.resetW' in the ed
      * call write_data()
      * Wait for the callback
      * Read the log var 'memTst.errCntW' from the ed and validate that it
        is 0
    """
...
class _ReadRequest:
    ...
    def __init__(self, mem, addr, length, ed):
    ...
    def add_data(self, addr, data):
        """Callback when data is received from the Espdrone"""
        data_len = len(data)
...
class _WriteRequest:
    ...
    def __init__(self, mem, addr, data, ed):
        """Initialize the object with good defaults"""
        self.mem = mem
        self.addr = addr
