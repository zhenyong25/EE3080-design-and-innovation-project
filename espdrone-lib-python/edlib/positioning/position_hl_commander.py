...
Espdrone around. Some sort of positioning support is required, for
...
class PositionHlCommander:
    ...
    def __init__(self, espdrone,
        ...
        :param espdrone: a Espdrone or SyncEspdrone instance
    ...
    def take_off(self, height=DEFAULT, velocity=DEFAULT):
        ...
        if not self._ed.is_connected():
            raise Exception('Espdrone is not connected')

        self._is_flying = True
        self._reset_position_estimator()
        self._activate_controller()
        self._activate_high_level_commander()
        self._hl_commander = self._ed.high_level_commander
    ...
    def _reset_position_estimator(self):
        self._ed.param.set_value('kalman.initialX', '{:.2f}'.format(self._x))
        self._ed.param.set_value('kalman.initialY', '{:.2f}'.format(self._y))
        self._ed.param.set_value('kalman.initialZ', '{:.2f}'.format(self._z))

        self._ed.param.set_value('kalman.resetEstimation', '1')
        time.sleep(0.1)
        self._ed.param.set_value('kalman.resetEstimation', '0')
        time.sleep(2)

    def _activate_high_level_commander(self):
        self._ed.param.set_value('commander.enHighLevel', '1')

    def _activate_controller(self):
        value = str(self._controller)
        self._ed.param.set_value('stabilizer.controller', value)
