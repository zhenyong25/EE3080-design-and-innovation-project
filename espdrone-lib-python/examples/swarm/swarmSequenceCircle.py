...
#  MA  02110-1301, USA.
"""
A script to fly 5 Espdrones in formation. One stays in the center and the
...
def poshold(ed, t, z):
    ...
    for r in range(steps):
        ed.commander.send_hover_setpoint(0, 0, 0, z)
        time.sleep(0.1)


def run_sequence(sed, params):
    ed = sed.ed
    ...
    ramp = fs * 2
    for r in range(ramp):
        ed.commander.send_hover_setpoint(0, 0, 0, base + r * (z - base) / ramp)
        time.sleep(fsi)
